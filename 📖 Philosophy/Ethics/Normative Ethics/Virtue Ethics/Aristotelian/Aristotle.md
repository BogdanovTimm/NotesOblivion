...
- To assert ontological priority for virtue over right action is to claim that the rightness of actions depends on their being produced by virtues, so that (1) an action is right only if it is done by a virtuous agent, and (2) the agent’s being virtuous explains the act’s being right, and the reverse is not true.